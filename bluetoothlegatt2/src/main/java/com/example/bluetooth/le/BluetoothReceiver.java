...
public class BluetoothReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.e("TAG","summer action1="+ action);
        ...
        if (BluetoothDevice.ACTION_FOUND.equals(action)) {  //发现设备
            ...
            if (btDevice.getName() !=null && btDevice.getName().contains(Constans.BT_NAME))//HC-05设备如果有多个，第一个搜到的那个会被尝试。
            {
                if (btDevice.getBondState() == BluetoothDevice.BOND_NONE) {
                    ...
                    try {
                        ...
                        boolean result = ClsUtils.createBond(btDevice.getClass(), btDevice);
                        Log.e("ywq", "result :" + "[" + result+ "]");
                    } catch (Exception e) {
        ...
        } else if (BluetoothDevice.ACTION_PAIRING_REQUEST.equals(action)) //再次得到的action，会等于PAIRING_REQUEST
        {
            ...
            if (btDevice.getName().contains(Constans.BT_NAME)) {
                Log.e("here", "OKOKOK");
